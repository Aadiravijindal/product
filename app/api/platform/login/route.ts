...
export async function POST(req: NextRequest) {
  ...
  const role = checkLogin(body.email ?? '', body.passcode ?? '');
  if (!role) {
    return NextResponse.json({ error: 'Wrong email or passcode.' }, { status: 401 });
  }
  const email = body.email!.trim().toLowerCase();
  await appendAudit(email, `Console sign-in (role: ${role})`, 'enterprise-console');
  const res = NextResponse.json({ ok: true, role });
  res.cookies.set(COOKIE, platformToken(email, role), {
    httpOnly: true,
    sameSite: 'lax',
    path: '/',
    maxAge: 60 * 60 * 24 * 7,
