...
export async function POST(_req: NextRequest, ctx: { params: Promise<{ scanId: string; findingId: string }> }) {
  ...
  if (claudeAvailable()) {
    try {
      ...
      analysis = {
        engine: 'builtin',
        engineNote: `Live Claude call failed (${err instanceof Error ? err.message.slice(0, 120) : 'error'}) — served from the built-in remediation library. Retry to call the API again.`,
